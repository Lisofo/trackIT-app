...
class ConsumosScreenState extends State<ConsumosScreen> {
  ...
  Map<String, bool> materialesSeleccionados = {}; // Para checkboxes
  ...
  // NUEVO MÉTODO: Inicializar selección de materiales
    ...
    for (var entryMaterial in cambiosPendientesLote.entries) {
      ...
      for (var entryReferencia in cambiosPorReferencia.entries) {
        ...
        await _procesarUnCambioLote(
          fila.itemId, referencia, nuevoValor, token, loteId
        );
      }
    }
      ...
      for (var entryOrden in cambiosPorOrden.entries) {
        ...
        try {
          ...
          if (lineaOriginal != null) {
            final lineaActualizada = lineaOriginal.copyWith(control: nuevoValor);
  ...
  // ============================================
  // NUEVO: MÉTODOS PARA FILAS 25 Y 26
  // ============================================
  
  // NUEVO MÉTODO: Calcular valores de las filas 25 y 26
  Map<String, Map<String, double>> _calcularFilasMultiplicacion() {
    ...
    final porcentajes = _tablaConsumo.calcularPorcentajes();
    
    // Para cada columna (ANT1-4 y órdenes)
    for (int i = 1; i <= 4; i++) {
      final columna = 'ant_$i';
      double sumaPorcentajesSeleccionados = 0.0;
      
      // Sumar porcentajes de materiales seleccionados
      for (var fila in _tablaConsumo.filas) {
        if (materialesSeleccionados[fila.codigo] == true) {
          sumaPorcentajesSeleccionados += porcentajes[fila.codigo]?[columna] ?? 0.0;
        }
      }
    ...
    // Hacer lo mismo para las columnas de órdenes
    for (var orden in _ordenesSeleccionadas) {
      final columna = orden.ordenTrabajoId.toString();
      double sumaPorcentajesSeleccionados = 0.0;
  ...
  // NUEVO MÉTODO: Seleccionar/deseleccionar todos los materiales
  void _toggleTodosMateriales(bool seleccionar) {
    setState(() {
      for (var key in materialesSeleccionados.keys) {
        materialesSeleccionados[key] = seleccionar;
      }
    });
  }
  
  // NUEVO MÉTODO: Actualizar multiplicadores
  ...
  // ============================================
  // NUEVO: WIDGET PARA CONTROLES DE MULTIPLICACIÓN
  // ============================================
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Seleccione los materiales y defina los valores para las filas de cálculo:',
            ...
            Row(
              children: [
                ...
                Row(
                  children: [
                    Checkbox(
                      value: materialesSeleccionados.isNotEmpty && materialesSeleccionados.values.every((value) => value),
  ...
  // ============================================
  // NUEVO: TABLA DE PORCENTAJES CON CHECKBOXES Y FILAS 25, 26
  // ============================================
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                const Text(
                  'Porcentaje que representa cada material en el total de cada columna',
                  style: TextStyle(fontSize: 12, color: Colors.grey),
                ),
                const SizedBox(height: 16),
                LayoutBuilder(
                  builder: (context, constraints) {
                    ...
                    final double anchoMinimoRequerido = 60 + 80 + 150 + (4 * 70) + (numOrdenes * 100) + 20;
                      ...
                      child: ConstrainedBox(
                        ...
                        child: SizedBox(
                          ...
                          child: DataTable(
                            ...
                            dataRowHeight: 50,
                            headingRowHeight: 60,
                            columns: _buildColumnasPorcentajes(),
                            rows: _buildFilasPorcentajes(porcentajes, filasMultiplicacion),
                          ),
                        ),
                      ),
                    );
  ...
  List<DataColumn> _buildColumnasPorcentajes() {
  ...
  List<DataRow> _buildFilasPorcentajes(
    Map<String, Map<String, double>> porcentajes,
    ...
    for (var fila in _tablaConsumo.filas) {
      ...
      final celdas = <DataCell>[
        ...
        DataCell(
          Center(
            child: Checkbox(
              value: materialesSeleccionados[codigo] ?? false,
              onChanged: (value) {
                setState(() {
                  materialesSeleccionados[codigo] = value ?? false;
                });
              },
            ),
          ),
        ...
        DataCell(Align(
          ...
          child: Text(codigo, style: const TextStyle(fontSize: 14))
        )),
        DataCell(Align(
          alignment: Alignment.centerLeft, 
          child: Tooltip(
        ...
        for (int i = 1; i <= 4; i++)
          DataCell(
            SizedBox(
              width: 70,
              child: Center(
                child: _buildCeldaPorcentaje(
                  porcentajesFila['ant_$i'] ?? 0.0,
                  fila.consumosAnteriores['ant_$i'] ?? 0.0
                ),
              ),
            ),
          ),
        ...
        for (var orden in _ordenesSeleccionadas)
          DataCell(
            SizedBox(
              ...
              child: Center(
                child: _buildCeldaPorcentaje(
                  porcentajesFila[orden.ordenTrabajoId.toString()] ?? 0.0,
                  fila.consumosPorOrden[orden.ordenTrabajoId.toString()] ?? 0.0
                ),
              ),
            ),
          ),
  ...
  Widget _buildCeldaPorcentaje(double porcentaje, double valorAbsoluto) {
    ...
    return Tooltip(
      message: '${valorAbsoluto.toStringAsFixed(0)} unidades\n'
              '${porcentaje.toStringAsFixed(1)}% del total de la columna',
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Text(
          ...
          if (valorAbsoluto > 0)
            Text(
              '(${valorAbsoluto.toStringAsFixed(0)})',
              style: const TextStyle(
                fontSize: 10,
                color: Colors.grey,
              ),
  ...
  // ============================================
  // NUEVO: MÉTODO _buildCeldaPorcentajeCalculado
  // ============================================
